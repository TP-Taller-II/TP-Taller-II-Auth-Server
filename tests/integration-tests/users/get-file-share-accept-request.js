'use strict';

const assert = require('assert');
const chai = require('chai');
const chaiHttp = require('chai-http');
const sandbox = require('sinon').createSandbox();

const { TokenServices } = require('../../../src/services');
const Model = require('../../../src/databases/mongodb/model');
const STATUS_CODES = require('../../../src/utils/status-codes.json');

process.env.PORT = 3030;

const app = require('../../../src');

chai.use(chaiHttp);

describe('Users', async () => {

	beforeEach(() => {
		process.env.PORT = 3030;
		sandbox.stub(TokenServices.prototype, 'verifyToken').resolves({ _id: '60456ebb0190bf001f6bbee2' });

	});

	afterEach(() => {
		delete process.env.PORT;
		sandbox.restore();
	});

	const shareRequest = {
		_id: ('60465d7ff28dcf001ff6dd70'),
		accepted: true,
		hostEmail: 'apiro@fi.uba.ar',
		guestEmail: 'ariel.piro@hotmail.com',
		fileId: ('60465d25f28dcf001ff6dd6f'),
		canShare: true,
	};

	const guest = {
		_id: '60456ebb0190bf001f6bbee2',
		canShare: [],
		canRead: [],
		name: 'Ariel',
		surname: 'Piro Martino',
		birthDate: '1996-08-03T00:00:00.000Z',
		email: 'ariel.piro@hotmail.com',
		password: '$2b$10$FVLh9oI6betv13edzE9cQuNbXVFqTu3pp3MfKP9mp9Uv/rVXQuDf6',
		provider: 'email',
		profilePic: 'https://firebasestorage.googleapis.com/v0/b/tddrive-b11e3.appspot.com/o/8a909882b3c6a2fcba6e1d4a42dabd42.jpg',
		createdAt: '2021-03-08T00:24:27.083Z',
		updatedAt: '2021-03-09T21:43:10.726Z',
		__v: 0,
	};

	describe('accept share request', async () => {

		it('Should set status code 200 when it can accept the share request (can share mode)', async () => {

			sandbox.stub(Model.prototype, 'update').resolves(true);
			sandbox.stub(Model.prototype, 'find').resolves([shareRequest]);
			sandbox.stub(Model.prototype, 'findBy').resolves([guest]);

			const res = await chai.request(app).get('/api/users/fileShareAcceptRequest/60456ebb0190bf001f6bbee2')
				.set('x-auth-token', 'some-valid-token');
			assert.deepStrictEqual(res.status, STATUS_CODES.OK);
			assert.deepStrictEqual(res.body, { message: 'The file share invitation was accepted' });

			sandbox.assert.calledWithExactly(Model.prototype.update, { _id: '60456ebb0190bf001f6bbee2' }, { accepted: true });
			sandbox.assert.calledWithExactly(Model.prototype.update, { email: guest.email }, { canShare: [shareRequest.fileId] });
			sandbox.assert.calledOnceWithExactly(Model.prototype.find, { _id: '60456ebb0190bf001f6bbee2' });
		});

		it('Should set status code 200 when it can accept the share request (can read mode)', async () => {

			sandbox.stub(Model.prototype, 'update').resolves(true);
			sandbox.stub(Model.prototype, 'find').resolves([{ ...shareRequest, canShare: false }]);
			sandbox.stub(Model.prototype, 'findBy').resolves([guest]);

			const res = await chai.request(app).get('/api/users/fileShareAcceptRequest/60456ebb0190bf001f6bbee2')
				.set('x-auth-token', 'some-valid-token');
			assert.deepStrictEqual(res.status, STATUS_CODES.OK);
			assert.deepStrictEqual(res.body, { message: 'The file share invitation was accepted' });

			sandbox.assert.calledWithExactly(Model.prototype.update, { _id: '60456ebb0190bf001f6bbee2' }, { accepted: true });
			sandbox.assert.calledWithExactly(Model.prototype.update, { email: guest.email }, { canRead: [shareRequest.fileId] });
			sandbox.assert.calledOnceWithExactly(Model.prototype.find, { _id: '60456ebb0190bf001f6bbee2' });
		});

		it('Should set status code 500 when database fails', async () => {

			sandbox.stub(Model.prototype, 'update').rejects(new Error('DB ERROR'));
			sandbox.stub(Model.prototype, 'find').resolves(true);

			const res = await chai.request(app).get('/api/users/fileShareAcceptRequest/60456ebb0190bf001f6bbee2')
				.set('x-auth-token', 'some-valid-token');

			assert.deepStrictEqual(res.status, STATUS_CODES.INTERNAL_SERVER_ERROR);
			assert.deepStrictEqual(res.body, { message: 'DB ERROR' });

			sandbox.assert.calledOnce(Model.prototype.update);
			sandbox.assert.notCalled(Model.prototype.find);
		});
	});
});

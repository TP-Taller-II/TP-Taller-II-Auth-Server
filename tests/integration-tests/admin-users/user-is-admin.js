...
describe('AdminUsers', async () => {
	...
	describe('User Is Admin', async () => {
		...
		it('Should set status code 401 when no token is provided', async () => {
			...
			const res = await chai.request(app).get(`/auth-server/v1/admin/${fakeAdminUser._id}`);
		...
		it('Should set status code 200 when user is valid', async () => {
			...
			assert.deepStrictEqual(res.body, { id: fakeAdminUser._id });
		...
		it('Should set status code 403 when admin user is not found', async () => {
			...
			assert.deepStrictEqual(res.body, { message: 'User is not and admin.' });
